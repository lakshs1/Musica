...
class YouTubeService:
    ...
    async def search_music(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        if not self.api_key or self.api_key.startswith("your_"):
            return await self._scrape_youtube_search(query, limit)
        ...
        try:
            ...
            return self._serialize_video_items(details_payload.get("items", []), limit=limit, enforce_filter=False)
        except Exception:
            return await self._scrape_youtube_search(query, limit)

    async def get_music_recommendations(self, limit: int = 12) -> list[dict[str, Any]]:
        if not self.api_key or self.api_key.startswith("your_"):
            return await self._scrape_youtube_search("trending music audio", limit)
    ...
    async def _scrape_youtube_search(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        import json
        import re
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                response = await client.get(url, params={"search_query": query}, headers=headers)
                    ...
                    if isinstance(obj, dict):
                        if "videoRenderer" in obj:
                            ...
                            if video_id and title_text:
                                ...
                                if len(parts) == 1:
                                    iso_duration = f"PT{parts[0]}S"
                                elif len(parts) == 2:
                                    iso_duration = f"PT{parts[0]}M{parts[1]}S"
                                elif len(parts) == 3:
                                    iso_duration = f"PT{parts[0]}H{parts[1]}M{parts[2]}S"
                                else:
                                    iso_duration = "PT0S"
